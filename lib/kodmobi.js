...
export default class Kodmobi extends Event {
	...
	get channels() {
		return this.#storage?.channel?.list || [];
	}
	...
	async create(to, send) {
		try {
			...
			if (isNew) {
				this.emit(EVENT_SESSION_CREATED, { ...this.#storage.channel });
			}
	...
	async send(channel) {
		...
		if (data.timeout > 0) {
			this.#timer.startChannelTimer(data.type, data.timeout);
		}
	...
	async check(code) {
		...
		this.emit(EVENT_SESSION_SUCCESS, { ...(result.data || []) });
